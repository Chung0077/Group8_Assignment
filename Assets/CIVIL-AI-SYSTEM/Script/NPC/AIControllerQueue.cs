...
namespace AISystem
{
    ...
    public class AIControllerQueue
    {
        ...
        public void RemoveCandidate(AI_CONTROLLER controller)
        {
            for(int i = 0; i < candidates.Count; i++)
            {
                if (candidates[i].GetController() == controller)
                {
        ...
        {
            float highest = -100;
            AI_CONTROLLER controller = AI_CONTROLLER.NONE;
            ...
            foreach(var candidte in candidates)
            {
                if(highest < candidte.GetWeight())
                {
                    ...
                    controller = candidte.GetController();
                }
            }
