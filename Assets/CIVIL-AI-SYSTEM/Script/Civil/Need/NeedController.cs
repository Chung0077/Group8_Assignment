...
namespace AISystem.Civil.NeedSystem
{
    public class NeedController : Controller
    {
        ...
        public override void Manage(AIDataBoard databoard, float currentTime)
        {
            ...
            // Set action
            databoard.currentAction = (Action)state.node[1];
            ...
            bool itemNeededMet = Common.Check.CheckItemNeededIsMet(databoard);

            if (result.Value == true && itemNeededMet == true)
            {
                // Handle Action Requirement
                bool? actionRequirementMet = Common.Check.ApplyActionRequirement(nodeIterator[nodeIterator.Length - 1].Current(), this, databoard);
